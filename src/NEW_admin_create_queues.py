...
if __name__ == "__main__":
    ...
    for i in range(n_switches): # for every switch
        ...
        for port in switch_ports: # for every port of the switch
            ...
            for slice_key in slices_defined.keys(): #for ecery slice
                # check if the slice involve the current port of the given switch
                if str(switch_id) in slices_defined[slice_key]["link_capacity"].keys() and str(switches_ports[switch_id][port]) in slices_defined[slice_key]["link_capacity"][str(switch_id)]:
                    queue_capacity = slices_defined[slice_key]["link_capacity"][str(switch_id)][str(switches_ports[switch_id][port])]
                    ...
                    if queue_capacity != 0 :
                        ...
                        # define queue usage
                        slice_application_ports = get_ports_from_slice(port_to_slice, int(slice_key))

                        queue_usage = ""

                        port_to_host = {v: k for k, v in topology["hosts_macs_to_switches_ports"][str(switch_id)].items()}
                        
                        host_involved = str(convert_mac_to_host_id(port_to_host[port]))

                        for app_port in slice_application_ports:
                            if app_port == "DEFAULT":
                                ...
                                queue_usage = f"sudo ovs-ofctl add-flow s{switch_id} ip,priority=65500,nw_src={convert_int_to_ip(int(switch_id))},nw_dst={convert_int_to_ip(switches_ports[switch_id][port])},tp_dst={app_port},idle_timeout=0,actions=set_queue:{queue_counter},normal"
                                queues_usage.append(queue_usage)
