    ...
    for i in range(n_switches): # for every switch
            ...
            for port in switch_port_to_mac[switch]:
                ...
                for _slice in slice_details:
                    if connected_host_id in slice_details[_slice]["hosts"]:
                        ...
                        queues_rate.append("--id=@slice_"+_slice+" create queue other-config:min-rate=10000 other-config:max-rate=200000000")
        ...
        if switch in switch_port_to_switch:
          for port in switch_port_to_switch[switch]:
                ...
                link_capacity = links_type_to_capacity[link_type]
                queues_list=[]
                queues_rate=[]

                q_def = ("sudo ovs-vsctl set port s"+switch+"-eth"+port+" qos=@newqos -- \\\n"+
                "--id=@newqos create QoS type=linux-htb \\\n"+
                "other-config:max-rate="+str(link_capacity*1000*1000)+" \\\n")
            
                for _slice in slice_details:
                    if is_link_used(slice_details[_slice]["path_between_host"], (int(switch),int(connected_switch_id))):
                        queues_list.append("queues:"+_slice+"=@slice_"+_slice)
                        queues_rate.append("--id=@slice_"+_slice+" create queue other-config:min-rate=10000 other-config:max-rate="+str(slice_details[_slice]["link_capacity"][str(switch)][str(connected_switch_id)]*1000*1000))
                ...
                if not str(host_1) == str(host_2):
                    for switch in slice_details[_slice]["switches"]:
                        ...
                        if not slice_to_port[str(_slice)] == "DEFAULT":
                            pass
                            # rule = "sudo ovs-ofctl add-flow s"+str(hosts_to_switches_map[str(host_1)])+" tcp,priority=65500,nw_src="+convert_int_to_ip(int(host_1))+",nw_dst="+convert_int_to_ip(int(host_2))+",tp_dst="+str(slice_to_port[str(_slice)])+",idle_timeout=0,actions=set_queue:"+_slice+",normal"
                            # rule = rule + "\n"
                            # rule = rule + "sudo ovs-ofctl add-flow s"+str(hosts_to_switches_map[str(host_1)])+" udp,priority=65500,nw_src="+convert_int_to_ip(int(host_1))+",nw_dst="+convert_int_to_ip(int(host_2))+",tp_dst="+str(slice_to_port[str(_slice)])+",idle_timeout=0,actions=set_queue:"+_slice+",normal"
                        else:
